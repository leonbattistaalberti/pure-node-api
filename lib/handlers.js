const helpers = require("./helpers");
const _data = require("./data");
// request handler
let handlers = {};
// user handler
handlers.users = (data, callback) => {
  console.log(data.method);
  // acceptable request methods
  let acceptableMethods = ["get", "post", "put", "delete"];
  if (acceptableMethods.indexOf(data.method) > -1) {
    // if the method exists call the function related to that method
    handlers._users[data.method](data, callback);
  } else {
    callback(405);
  }
};
// _users object
handlers._users = {};
// _users post method
// data: firstName, lastName, phoneNumber, password, tosAgreement
handlers._users.post = (data, callback) => {
  let firstName =
    typeof data.payload.firstName == "string" &&
    data.payload.firstName.trim().length > 0
      ? data.payload.firstName.trim()
      : false;

  let lastName =
    typeof data.payload.lastName == "string" &&
    data.payload.lastName.trim().length > 0
      ? data.payload.lastName.trim()
      : false;

  let phoneNumber =
    typeof data.payload.phoneNumber == "string" &&
    data.payload.phoneNumber.trim().length == 10
      ? data.payload.phoneNumber.trim()
      : false;

  let password =
    typeof data.payload.password == "string" &&
    data.payload.password.trim().length > 0
      ? data.payload.password.trim()
      : false;

  let tosAgreement =
    typeof data.payload.tosAgreement == "boolean" &&
    data.payload.tosAgreement == true
      ? data.payload.tosAgreement
      : false;

  //check if all the required fields are filled
  if (firstName && lastName && phoneNumber && password && tosAgreement) {
    // check if the user exists
    _data.read("users", phoneNumber, (err, data) => {
      if (err) {
        // hash password
        // takes the password sent in the payload
        let hashedPassword = helpers.hash(password);
        // TODO: check if the password was hashed successfully

        // create the user object
        let userObject = {
          firstName: firstName,
          lastName: lastName,
          phoneNumber: phoneNumber,
          hashedPassword: hashedPassword,
          tosAgreement: true,
        };

        _data.create("users", phoneNumber, userObject, (err) => {
          if (err) {
            console.log(err);
            callback(500, { error: `Failed to create the user` });
          } else {
            callback(200);
          }
        });
      } else {
        callback(400, { error: `User already exists` });
      }
    });
  }
};

// _users get method
// Only authenticated users can access their data
handlers._users.get = (data, callback) => {
  // check if phone number is valid
  let phoneNumber =
    typeof data.queryStringObject.phone == "string" &&
    data.queryStringObject.phone.trim().length == 10
      ? data.queryStringObject.phone.trim()
      : false;
  if (phoneNumber) {
    _data.read("users", phoneNumber, (err, data) => {
      if (!err && data) {
        // remove hashedPassword in user data object
        delete data.hashedPassword;
        callback(200, data);
      } else {
        callback(404, {
          error: "phone number not matched - user is not authenticated",
        });
      }
    });
  } else {
    callback(404, { error: "missing required field" });
  }
};

// _users put method
handlers._users.put = (data, callback) => {
  // check if phone number is valid
  let phoneNumber =
    typeof data.payload.phoneNumber == "string" &&
    data.payload.phoneNumber.trim().length == 10
      ? data.payload.phoneNumber.trim()
      : false;
  // optional fields
  let firstName =
    typeof data.payload.firstName == "string" &&
    data.payload.firstName.trim().length > 0
      ? data.payload.firstName.trim()
      : false;

  let lastName =
    typeof data.payload.lastName == "string" &&
    data.payload.lastName.trim().length > 0
      ? data.payload.lastName.trim()
      : false;

  let password =
    typeof data.payload.password == "string" &&
    data.payload.password.trim().length > 0
      ? data.payload.password.trim()
      : false;

  if (phoneNumber) {
    // check if there is data to be updated
    if (firstName || lastName || password) {
      _data.read("users", phoneNumber, (err, userData) => {
        if (firstName) {
          userData.firstName = firstName;
        }
        if (lastName) {
          userData.lastName = lastName;
        }
        if (password) {
          userData.password = helpers.hash(password);
        }
        // store updated data
        _data.update("users", phoneNumber, userData, (err) => {
          if (!err) {
            callback(200);
          } else {
            callback(404);
          }
        });
      });
    } else {
      callback(404, { error: `user data was not updated` });
    }
  }
};

// _users delete method
handlers._users.delete = (data, callback) => {
  let phoneNumber =
    typeof data.payload.phoneNumber == "string" &&
    data.payload.phoneNumber.trim().length == 10
      ? data.payload.phoneNumber.trim()
      : false;

  if (phoneNumber) {
    _data.read("users", phoneNumber, (err) => {
      if (!err && data) {
        _data.delete("users", phoneNumber, (err) => {
          if (err) {
            callback(200, { msg: `user deleted successfully` });
          } else {
            callback(404, { error: err });
          }
        });
      } else {
        callback(505, { error: err });
      }
    });
  } else {
    callback(404, { error: `phone number not found` });
  }
};

/*
 *   Tokens Handlers
 */
handlers.token = (data, callback) => {
  // acceptable request methods
  let acceptableMethods = ["get", "post", "put", "delete"];
  if (acceptableMethods.indexOf(data.method) > -1) {
    // if the method exists call the function related to that method
    handlers._token[data.method](data, callback);
  } else {
    callback(405);
  }
};

// token object
handlers._token = {};

// _token post method
handlers._token.post = (data, callback) => {
  let phoneNumber =
    typeof data.payload.phoneNumber == "string" &&
    data.payload.phoneNumber.trim().length == 10
      ? data.payload.phoneNumber.trim()
      : false;

  let password =
    typeof data.payload.password == "string" &&
    data.payload.password.trim().length > 0
      ? data.payload.password.trim()
      : false;

  console.log(`${phoneNumber} :: ${password}`);
  if (phoneNumber && password) {
    _data.read("users", phoneNumber, (err, userData) => {
      if (!err && userData) {
        let hashedPassword = helpers.hash(password);
        if (hashedPassword === userData.hashedPassword) {
          let tokenId = helpers.createRandomString(20);

          let tokenExp = Date.now() + 1000 * 60 * 60;

          let tokenOject = {
            phone: phoneNumber,
            id: tokenId,
            tokenExpiration: tokenExp,
          };

          _data.create("tokens", tokenId, tokenOject, (err) => {
            if (!err) {
              callback(200, { msg: `token created`, token: tokenOject });
            } else {
              callback(404, { error: "Unable to create token" });
            }
          });
        } else {
          callback(404, { error: "password does not match" });
        }
      }
    });
  } else {
    callback(404, { error: "user not found" });
  }
};
// _token get handler
handlers._token.get = (data, callback) => {
  // check if phone number is valid
  let tokenid =
    typeof data.queryStringObject.tokenid == "string" &&
    data.queryStringObject.tokenid.trim().length > 0
      ? data.queryStringObject.tokenid.trim()
      : false;
  console.log(tokenid);
  if (tokenid) {
    _data.read("tokens", tokenid, (err, tokenData) => {
      if (!err && tokenData) {
        // remove hashedPassword in user data object
        callback(200, tokenData);
      } else {
        callback(404, {
          error: "token not found",
        });
      }
    });
  } else {
    callback(404, { error: "missing required field" });
  }
};

// ping route handler
handlers.ping = (data, callback) => {
  //callback http status and the payload if there is one
  callback(200);
};

// films route handler
handlers.films = (data, callback) => {
  callback(406, {
    films: ["blue velvet", "twin peaks"],
  });
};

module.exports = handlers;
